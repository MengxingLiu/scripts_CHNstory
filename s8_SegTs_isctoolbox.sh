...
do
    ...
    do
        ...
        echo "========================="$sub"===================================="
        echo "================================================================="
        3dTcat $spath/analysis/ISC/preprocess/$sub.results/pb04."$sub".r*.scale+tlrc. -prefix $spath/analysis/ISC/preprocess/$sub.results/pb04."$sub".scale.nii
        # cp $spath/data/$sub.results/pb05."$sub"."$hemi".scale.niml.dset $spath/analysis/ISC/tmpfile
        for type in NS CS US SW
        do
            3dTcat $spath/analysis/ISC/preprocess/pb04."$sub".scale.nii"[1dcat $indexpath/index_$type.1D]" \
                            -prefix $spath/analysis/ISC/preprocess/pb04."$sub"."$type".scale.nii
        done
    done
done    
